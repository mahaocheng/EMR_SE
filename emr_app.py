...
stopw = [line.strip().decode('utf-8') for line in open('./extra_dict/stop_words.txt').readlines()]
...
class ChineseTokenizer(Tokenizer):
    ...
    def __call__(self, value, positions=False, chars=False,
        ...
        list_seg = list(set(jieba.cut(value, cut_all=True))-set(stopw))
        for w in list_seg:
            t.original = t.text = w
            t.boost = 0.5
            if positions:
...
while True:
    ...
    with ix.searcher() as searcher:
        query = qparser.QueryParser("FJBNAME", ix.schema ,group=qparser.OrGroup).parse("".join(jieba.cut1(myquery, cut_all=True)))
        results = searcher.search(query)
        FPRN_list = []
        for hit in results:
