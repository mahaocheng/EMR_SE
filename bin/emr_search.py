...
outfile.close()
"""
# load jieba dict which contains medical words
jieba.load_userdict(open("/home/haohedata/github/EMR_SE/dict/dict.txt"))
# create a list which contains chinese stop words.
#stopw = [line.strip().decode('utf-8') for line in open('/home/haohedata/github/EMR_SE/dict/stop_words.txt').readlines()]

#words = jieba.cut_for_search(u"糖尿病并眼部病变")
#print '/'.join(words)

#define a chinese analyzer.
"""
class ChineseTokenizer(Tokenizer):
    def __call__(self, value, positions=False, chars=False,
        ...
        list_seg = list(set(jieba.lcut(value, cut_all=True))-set(stopw))
        for w in list_seg:
            t.original = t.text = w
            t.boost = 0.5
            if positions:
...
# connect db of yanjiao hospital
ms = MSSQL(host="192.168.1.231",user="sa",pwd="1",db="master")
#fetch data by executing sql query
...
while True:
    ...
    #myquery = list(set(jieba.cut(myquery,cut_all=True)-set(stopw))
    #print myquery

    with ix.searcher() as searcher:
        query = qparser.QueryParser("FJBNAME", ix.schema ,group=qparser.OrGroup).parse("".join(jieba.lcut(myquery, cut_all=True)))
        results = searcher.search(query)
        FPRN_list = []
        for hit in results:
